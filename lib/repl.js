...
export default class MercuryRepl {
	...
	eval(){
		...
		try {
			...
			if (editor){
				...
				// flashes red when an error occured and returns
				this.flash(error);
				// if error, exit
				if (error) return;

				// if there are any warnings, unfold the window
				// print the warnings but still evaluate the code
				if (parse.warnings.length > 0 && this.showWarnings){
					parse.warnings.forEach((e) => {
						this.log(e);
					});
				}
	...
	flash(error) {
		...
		if (editor){
			...
			var decoration = editor.decorateMarker(
				marker, {
					type: 'line',
					// if code contains error, flash red instead of white
					class: error ? 'mercury-error' : 'mercury-flash'
				});
	...
	toggleWarnings(){
		// enable or disable the printing of warnings
		this.showWarnings = !this.showWarnings;
		this.log(`Warnings are ${this.showWarnings? 'enabled' : 'disabled'}`);
	}
